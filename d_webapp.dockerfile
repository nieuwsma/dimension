...
RUN set -ex && go build -v -o /usr/local/bin/webapp github.com/nieuwsma/dimension/cmd/web-gui
...
COPY --from=builder /usr/local/bin/webapp /usr/local/bin/.
COPY cmd/web-gui/public /usr/local/bin/public
COPY .version /
...
RUN chown -R 65534:65534 /usr/local/bin/public
